...
const Navbar = () => {
  ...
  return (
    <div className="flex items-center justify-between py-5 font-medium">
      {/* Logo */}
            {/* <Link to={localStorage.getItem("user") ? `/${JSON.parse(localStorage.getItem("user")).position.toLowerCase()}/home` : "/"}>
  <img src={assets.logo} className="w-25 rounded-xl" alt="logo_img" />
</Link> */}
      ...
      <ul className="hidden sm:flex gap-5 text-sm text-gray-700">
        {currentMenu.map((item) => (
          <li
            ...
            className={`${isActive(item.path) ? "text-blue-600 font-semibold" : "text-gray-700"} cursor-pointer`}
          >
            {item.label}
          </li>
        ))}
