...
const generateToken = (userId, position) => {
  return jwt.sign({ id: userId, role: position }, process.env.JWT_SECRET, { expiresIn: "7d", });
};
...
const signupUser = async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `INSERT INTO users (name, position, working_id, password) VALUES ($1, $2, $3, $4) RETURNING id, name, position, working_id, created_at`,
